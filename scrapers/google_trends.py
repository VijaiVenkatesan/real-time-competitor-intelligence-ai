...
def get_trends_data(company_name: str, timeframe: str = "today 3-m") -> dict:
    ...
    try:
        ...
        pytrends.build_payload([company_name], cat=0, timeframe=timeframe, geo="", gprop="")
        ...
        if not iot.empty and company_name in iot.columns:
            ...
            result["average_interest"] = round(sum(values) / len(values), 1) if values else 0
        ...
        if company_name in related:
            ...
            if rising is not None and not rising.empty:
                result["related_queries_rising"] = rising.head(10)["query"].tolist()
            if top is not None and not top.empty:
                result["related_queries_top"] = top.head(10)["query"].tolist()
